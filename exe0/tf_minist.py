...
def neural_network(data):
    ...
    layer_3_w_b = {'w_': tf.Variable(tf.random_normal(
        [n_layer_2, n_layer_3])), 'b_': tf.Variable(tf.random_normal([n_layer_3]))}
    # 定义输出层"神经元"的权重和biases
    layer_output_w_b = {'w_': tf.Variable(tf.random_normal(
        [n_layer_3, n_output_layer])), 'b_': tf.Variable(tf.random_normal([n_output_layer]))}
    ...
    layer_output = tf.add(tf.matmul(layer_3, layer_output_w_b[
                          'w_']), layer_output_w_b['b_'])
...
def train_neural_network(X, Y):
    ...
    cost_func = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
        logits=predict, labels=Y, name=None))
    ...
    with tf.Session() as session:
        ...
        print('准确率: ', accuracy.eval(
            {X: mnist.test.images, Y: mnist.test.labels}))
        print("Epoch: " + str(epochs) + ' 准确率: ' +
              str(accuracy.eval({X: mnist.test.images, Y: mnist.test.labels})), end="\n", file=fi)
