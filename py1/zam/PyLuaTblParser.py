...
class LuaTableReader:
    ...
    def back(self):
        if self.__prevp != -1:
            ...
            raise LuaSyntaxError('next_clean() should be called before back()')
    ...
    def next_table(self):
        ...
        c = self.next_clean()
        if c != '{':
            raise LuaSyntaxError('a table must start with \'{\'')
        while True:
            c = self.next_clean()
            if c is None:
                raise LuaSyntaxError('a table must end with \'}\'')
            elif c == '}':
                break
            else:
                self.back()
            ...
            if c in ',;':
                ...
                raise LuaSyntaxError('expected a \',\' \';\' or \'}\'')
            table += ','
        table += '}'
        return table, fields
    ...
    def __next_field(self):
        ...
        if c == '[':
            ...
            if ok:  # we get a xstring
                ...
                if x == ']':
                    if not self.__test_validity_of_index(index):
                        raise LuaSyntaxError('invalid table index : ' + index)
                    expr1 = index
                    if self.next_clean() != '=':
                        raise LuaSyntaxError('invalid table field')
            ...
            if x == '=':
                if not self.__test_validity_of_name(expr1):
                    raise LuaSyntaxError('invalid variable name : ' + expr1)
                expr2 = self.__next_expr()
                field = expr1 + '= ' + expr2
            elif x is not None:
                self.back()
        ...
        c = self.next_clean()
        if c not in ',;}':
            raise LuaSyntaxError('syntax error near \"' + expr2 + '\"')
        else:
            self.back()
    ...
    def __next_expr(self):
        ...
        if c == '{':
            ...
            if ok:
                ...
                raise LuaSyntaxError('syntax error near \'[\'')
        elif c is None:
            raise LuaSyntaxError('an expression cannot be empty')
        else:
            raise LuaSyntaxError('syntax error near \'' + c + '\'')
    ...
    def __next_string(self):
        ...
        while True:
            c = self.__next()
            if c is None:
                raise LuaSyntaxError('a string must end with \' or \"')
            elif c == mark:
                break
            elif c in '\'\"':
                ret += '\\' + c
    ...
    def __try_read_xstring(self):
        ...
        if c != '[':
            raise LuaSyntaxError('assertion failed')
    ...
    def __read_xstring_aux(self, n):
        ...
        while True:
            if c is None:
                raise LuaSyntaxError('invalid lua xstring')
            elif c == ']':
                coll, i = ']', n
                x = self.__next()
                while x is not None:
...
class PyLuaTblParser:
    ...
    def __dump_aux(self, d, indent_factor, indent):
        ...
        if length == 1:
            key = keys[0]
            ret += self._dump_index(key)
            ret += '='
            if indent_factor > 0:
                ret += ' '
            ret += self._dump_value(d[key], indent_factor, indent)
        elif length != 0:
            ...
            for key in keys:
                ...
                if indent_factor > 0:
                    ret += '\n'
                ret += self._indent(new_indent)
                ret += self._dump_index(key)
                ret += '='
                if indent_factor > 0:
                    ret += ' '
                ret += self._dump_value(d[key], indent_factor, new_indent)
                commanate = True
            if indent_factor > 0:
                ret += '\n'
    ...
    def _dump_index(self, index):
        if isinstance(index, (int, float)):
            return '[' + str(index) + ']'
        elif isinstance(index, str):
            return '[' + self._dump_string(index) + ']'
        else:
            return LuaSyntaxError('the table index must be a string or a number')

    def _dump_value(self, v, indent_factor, indent):
        if isinstance(v, bool):
            ...
            return self._dump_string(v)
        elif isinstance(v, list):
            return self._dump_list(v)
    ...
    def _dump_list(self, lst):
        commanate = False
        ret = '{'
        for elem in lst:
            if commanate:
                ret += ','
            ret += self._dump_value(elem, 0, 0)
            commanate = True
        ret += '}'
        return ret
